...
const Snack = () => {
  ...
  // Listen for the 'newSnack' event to update the snack list live
  socket.on('snackDoc', (snackDoc) => {
    setMenuItems((prevItems) => [...prevItems, snackDoc]);
    toast.success("A new snack has been added!");
  });

    // Listen for the 'updateSnack' event and update the snack list
    socket.on('updateSnack', (updatedSnack) => {
      setMenuItems((prevItems) => {
        return prevItems.map((item) =>
          item._id === updatedSnack._id ? updatedSnack : item
        );
      });
      toast.info("A snack has been updated!");
    });

    // Listen for the 'deleteSnack' event and remove the snack from the list
    socket.on('deleteSnack', ({ id }) => {
      setMenuItems((prevItems) => prevItems.filter((item) => item._id !== id));
      toast.error("A snack has been deleted!");
    });
  ...
  const fetchMenuItems = async () => {
    try {
      const response = await axios.get(`${baseUrl}/api/snacks/getSnacks`);
